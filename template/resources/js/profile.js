...
document.addEventListener("DOMContentLoaded", function () {
  const statsDropdown = document.querySelectorAll(".tab-content-stats-dropdown");
  const ctx = document.getElementById("viewsChart").getContext("2d");

  const viewDataSets = {
    weekly: [1500, 700, 900, 1200, 2313, 400, 1300],
    monthly: Array.from({ length: 30 }, () => Math.floor(Math.random() * 8000) + 1000), // Random 30 hari
    yearly: Array.from({ length: 12 }, () => Math.floor(Math.random() * 60000) + 5000) // Random 12 bulan
  };
    ...
    return new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          label: "Views",
          data: data,
          backgroundColor: data.map((val) => (val === Math.max(...data) ? "#ccff00" : "#D9D9D9")),
          borderRadius: 8,
          maxBarThickness: 25
        }]
      },
      options: {
        plugins: {
          ...
          tooltip: {
            ...
            padding: 10,
            cornerRadius: 6,
            bodyFont: { size: 16, family: "Poppins, Arial, sans-serif" }
          }
        },
        responsive: true,
        scales: {
  ...
  document.getElementById("timeViewFilter").addEventListener("change", function (e) {
    ...
    viewChart.data.datasets[0].backgroundColor = selectedData.map((val) => (val === Math.max(...selectedData) ? "#ccff00" : "#D9D9D9"));

    document.getElementById('view-count').innerText = Math.max(...selectedData);

    viewChart.options.scales.y.ticks.stepSize = getStepSize(selectedData);
    ...
    return new Chart(downloadsCtx, {
      ...
      data: {
        ...
        datasets: [{
          label: "Downloads",
          data: data,
          backgroundColor: data.map((val) => (val === Math.max(...data) ? "#ffcc00" : "#D9D9D9")),
          borderRadius: 8,
          maxBarThickness: 25
        }],
      },
  ...
  let downloadChart = createDownloadChart(downloadDataSets.weekly, labels.weekly);

  document.getElementById("timeDownloadFilter").addEventListener("change", function (e) {
    const selectedPeriod = e.target.value;
    ...
    downloadChart.data.datasets[0].backgroundColor = selectedData.map((val) => (val === Math.max(...selectedData) ? "#ffcc00" : "#D9D9D9"));
    document.getElementById('download-count').innerText = Math.max(...selectedData);
    downloadChart.options.scales.y.ticks.stepSize = getStepSize(selectedData);
